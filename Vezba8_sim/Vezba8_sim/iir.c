...
Int16 Nth_order_IIR(Int16 input, Int16 coefficients[][6], Int16 x_history[][2],
	...
	temp = input;
	/* for N stages */
	for ( stage = 0 ; stage < order/2 ; stage++){
		accum = 0;
		accum = _smac(accum, coefficients[stage][0], temp); 				/* A0 * x(n)      (x(n) is in temp)     */
		accum = _smac(accum, coefficients[stage][1], x_history[stage][0]);	/* A1/2 * x(n-1)  (x(n-1) is in x_historty[stage][0] */
		accum = _smac(accum, coefficients[stage][1], x_history[stage][0]);	/* A1/2 * x(n-1) */
		accum = _smac(accum, coefficients[stage][2], x_history[stage][1]);	/* A2 * x(n-2)   */
		accum = _smas(accum, coefficients[stage][4], y_history[stage][0]);	/* B1/2 * y(n-1) (y(n-1) is in y_historty[stage][0]*/
		accum = _smas(accum, coefficients[stage][4], y_history[stage][0]);	/* B1/2 * y(n-1) */
		accum = _smas(accum, coefficients[stage][5], y_history[stage][1]);	/* B2 * y(n-2)   */
...
{
	Int16 temp = second_order_IIR(input, IIR_all_pass_2nd_order, z_x, z_y);
	if (lowpass) {
		temp += z_x[1];
	} else {
		temp -= z_x[1];
